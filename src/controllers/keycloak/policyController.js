...
export const keycloakaddPolicy = async (req, res) => {
    ...
    const keycloakUrl = `${serverUrl}/admin/realms/${realm}/clients/${client_name_id}/authz/resource-server/policy`;
...
export const keycloakPolicyList = async (req, res) => {
    const token = await keycloakAccessToken();
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      msg: "Policy list retive successfuly.",
      data: response.data,
    });
  } catch (error) {
